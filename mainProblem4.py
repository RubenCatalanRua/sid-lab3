...
from utils import draw_history, draw_history_2
import sys

from qlearningPOGEMA import QLearningAgent
from reinforcePOGEMA import ReinforceAgent
...
if __name__ == '__main__':
    if len(sys.argv) < 2 or (sys.argv[1] not in ["q-learning", "reinforce"]):
        print("Usage: python3 mainProblem4.py [q-learning | reinforce]")
        sys.exit(1)
    exp_config = {
    ...
    problem = "problem_4"
    data_path = f"{problem}/{save_in}"
    if sys.argv[1] == "q-learning": 
        algorithms = [QLearningAgent(env = game, epsilon=exp_config["epsilon_max"],
                        learning_rate=exp_config["learning_rate"], gamma = exp_config["gamma"], agent_id = i,)
                      for i in range(game.num_agents)]
    else :
        algorithms = [ReinforceAgent(env = game,
                        learning_rate=exp_config["learning_rate"], gamma = exp_config["gamma"],
                        t_max = exp_config["episode_length"], agent_id = i)
                      for i in range(game.num_agents)]
    ...
    reward_per_epoch = []
    td_error_per_epoch = []
    reward_per_epoch_per_agent = [[] for _ in range(game.num_agents)]

    pbar = tqdm(range(exp_config["epochs"]))  # Barra de progreso
    for epoch in pbar:
        all_eval_rewards = []
        all_td_errors = []
        all_eval_rewards_per_agent = [[] for _ in range(exp_config["num_agents"])]
        ...
        for ep in range(exp_config["episodes_per_epoch"]):
            ...
            while not all(terminated) and not all(truncated):  # Hasta que acabe el episodio
                ...
                [algorithms[i].learn_from_episode(actions, rewards, states[i], obs_to_state(observations[i]))
                for i in range(game.num_agents)]
                # Actualizamos métricas
                train_rewards = [train_rewards[i] + rewards[i] for i in range(game.num_agents)]
                if sys.argv[1] == "q-learning":
                    all_td_errors.append(algorithms[0].td_error["TD_err"][-1])
        ...
        for ep in range(evaluation_episodes):
            ...
            while not all(terminated) and not all(truncated):  # Hasta que acabe el episodio
                states = [obs_to_state(observations[i]) for i in range(game.num_agents)]
                actions = tuple([algorithms[i].select_action(states[i], training=False)
                                 for i in range(game.num_agents)])
                observations, rewards, terminated, truncated, infos = env.step(actions)
            ...
            for agent_i in range(exp_config["num_agents"]):
                ...
                if epoch == len(pbar) - 1: env.save_animation(f"{exp_config['renders']}/{solution_concept_name}-map{ep}-agent{agent_i}-epoch{epoch}.svg",
                                   AnimationConfig(egocentric_idx=agent_i, show_border=True, show_lines=True))
            all_eval_rewards.append(sum(total_rewards))
            [all_eval_rewards_per_agent[i].append(total_rewards[i]) for i in range(exp_config["num_agents"])]
        ...
        [reward_per_epoch_per_agent[i].append(sum(all_eval_rewards_per_agent[i])) for i in range(exp_config["num_agents"])]
    draw_history_2(reward_per_epoch_per_agent, "Recompensa individual", data_path, f"individual_rewards")
    draw_history(reward_per_epoch, "Recompensa colectiva", data_path, f"join_rewards")
    if (sys.argv[1] == "q-learning"): draw_history(td_error_per_epoch, "TD Error", data_path, f"td_error")
